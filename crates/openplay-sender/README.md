...
The binary is structured in four modules:

**`main.rs`** — parses CLI arguments, initializes GStreamer and logging, loads `AppConfig`, and launches the egui window.

**`app.rs`** (`SenderApp`) — the egui application. Runs three mDNS browsers (OpenPlay, AirPlay, Miracast) and maintains the receiver list. Handles UI state: protocol selection, bitrate/framerate sliders, and the Cast/Stop button. Bridges the egui main thread to the tokio runtime for async casting operations.
